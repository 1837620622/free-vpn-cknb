  ...
  return (
    <SiteShell stats={stats}>
      <section className="pt-10 sm:pt-16 pb-8 sm:pb-12">
        <div className="eyebrow mb-3" style={{ color: NV_GREEN }}>日报</div>
        <h1 className="display text-[36px] sm:text-[56px] lg:text-[72px] text-white leading-[1.0]">
          每日动态
        </h1>
        <p className="mt-4 text-[14px] sm:text-[15px] text-white/60 max-w-2xl">
          按发布时间倒序，最新发布的在最上面。{getWindowText()}之外的条目已自动归档。
        </p>
      </section>

      {groups.map((g, i) => g.items.length > 0 && (
        <section key={g.key} className={`py-10 sm:py-14 ${i > 0 ? 'border-t border-white/8' : ''}`}>
          <header className="flex items-baseline justify-between mb-6 sm:mb-8">
            <div>
              <div className="eyebrow mb-2" style={{ color: NV_GREEN }}>0{i + 1}</div>
              <h2 className="display text-[24px] sm:text-[32px] lg:text-[40px] text-white leading-[1.0]">{g.label}</h2>
            </div>
            <span className="text-[12.5px] text-white/40 font-mono-num">{g.items.length} 条</span>
          </header>
      ...
      {groups.every((g) => g.items.length === 0) && (
        <div className="py-20 text-center text-white/45 text-[14px]">{getWindowText()}内暂无新条目</div>
      )}
    </SiteShell>
  );
}
